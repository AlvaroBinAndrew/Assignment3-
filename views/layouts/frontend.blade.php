...
<script type="text/javascript">
  ...
  $.ajax({
    ...
    success:function(data){
      if(data==0){
        ...
        window.location.href = "{{url('curhat')}}";
      }
      else{
        alert('Gagal!');
      }
 ...
 function postCurhat(){
  $.ajax({
    ...
    type:"post",
    data:
      $("#form-curhat").serialize()
    ,
    success:function(data){
      if(data==0){
        alert('Curhat berhasil!');
        window.location.href = "{{url('curhat')}}";
      }
      else{
        alert('Gagal!');
...
$(document).delegate(".btn-reply","click", function () {
     var curhat_id = $(this).data('id');
     $(".modal-body #curhat_id").val( curhat_id );
});
